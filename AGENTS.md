...
| Gemini area quiz (ungrounded, ~2048 tok) | quiz 30d told / 7d no-quiz verdict, key `v4:<area name>` — the area ALONE, from one shared `quizCacheKey` (src/types/quiz.ts) that server and device both call, so the session cache can no longer hit where the route would regenerate. The nearest twelve are MATERIAL, not identity, exactly as retold treats its article: sorted before they reach the prompt so the SET decides what gets written, never the feed's distance order. Turso ('quiz' kind) + per-process map + device session cache + single-flight. Refuses below 3 usable stories WITHOUT calling. NOTE the material is CLIENT-SUPPLIED and the digest that used to isolate a fabricated POST is gone with the bucket-keying — one crafted POST now writes what everyone in that area reads for 30 days (#303). Retold's area key carries no such trust: its source is fetched server-side | free tier; SHARED 300-calls/day breaker |
