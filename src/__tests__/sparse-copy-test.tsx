...
describe('HistoryBody count line', () => {
  test('sparse: says we looked further, honestly', async () => {
    ...
    const { getByText } = await render(<><HistoryBody center={center} onManualCenter={jest.fn()} /><FeedCountLine center={center} /></>);
    expect(
  ...
  test('dense: the everyday count line, untouched', async () => {
    ...
    const { getByText, queryByText } = await render(<><HistoryBody center={center} onManualCenter={jest.fn()} /><FeedCountLine center={center} /></>);
